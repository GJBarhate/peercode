...
const { eloCalculator } = require('../../utils/eloSystem');
const { ELO_FLOOR } = require('../../constants/elo.constants');
...
module.exports = function defineRatingUpdateJob(agenda) {
  agenda.define('rating-update', async (job) => {
    try {
      ...
      await Promise.all([
        User.updateOne({ _id: idA }, { $set: { elo: Math.max(ELO_FLOOR, newEloA) } }),
        User.updateOne({ _id: idB }, { $set: { elo: Math.max(ELO_FLOOR, newEloB) } }),
      ]);
    } catch (err) {
      logger.error('rating-update job failed:', err.message);
    }
  });
};
