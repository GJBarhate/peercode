...
function CollapsibleSection({ title, icon: Icon, iconColor = 'text-indigo-400', defaultOpen = true, children, badge }) {
 ...
 return (
 <div className="border border-border-default rounded-xl overflow-hidden bg-bg-surface/30">
 <button
 type="button"
 onClick={() => setOpen(o => !o)}
 className="w-full flex items-center gap-3 px-4 py-3 hover:bg-bg-surface/5 transition-colors text-left"
 >
 <div className={`w-7 h-7 rounded-lg bg-bg-elevated flex items-center justify-center flex-shrink-0`}>
 <Icon className={`w-4 h-4 ${iconColor}`} />
 </div>
 <span className="font-semibold text-sm text-text-primary flex-1">{title}</span>
 {badge && <span className="text-[11px] px-2 py-0.5 bg-bg-elevated text-text-muted rounded-full">{badge}</span>}
 {open ? <ChevronUp className="w-4 h-4 text-text-muted" /> : <ChevronDown className="w-4 h-4 text-text-muted" />}
 </button>
 {open && <div className="px-4 pb-4 space-y-4 border-t border-border-default pt-4">{children}</div>}
 </div>
 )
}

const inputCls = (err) =>
 `w-full px-3 py-2 bg-bg-surface border ${err ? 'border-red-500' : 'border-border-strong'} rounded-lg text-text-primary text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 placeholder-text-muted transition-colors`

const textareaCls = (err, mono = false) =>
 `w-full px-3 py-2 bg-bg-surface border ${err ? 'border-red-500' : 'border-border-strong'} rounded-lg text-text-primary ${mono ? 'text-xs font-mono' : 'text-sm'} focus:outline-none focus:ring-1 focus:ring-indigo-500 placeholder-text-muted resize-none transition-colors`
...
function LangTabs({ activeLang, setActiveLang }) {
 ...
 activeLang === lang ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'text-text-muted hover:text-text-secondary'
 }`}>
...
export default function AddEditProblemModal({ isOpen, onClose, problem, onSaved }) {
 ...
 <div key={i} className="bg-bg-surface/60 border border-border-default rounded-xl p-4 space-y-3">
 <div className="flex items-center justify-between">
 <span className="text-xs font-bold text-text-muted">Example {i + 1}</span>
 ...
 rows={3}
 className="w-full mt-1 px-3 py-2 bg-bg-elevated border border-border-strong rounded-lg text-text-primary text-xs font-mono focus:outline-none focus:ring-1 focus:ring-indigo-500 resize-none placeholder-text-muted"
 />
 </div>
 <div>
 <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Output</label>
 <textarea
 value={ex.output}
 ...
 <div>
 <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Explanation (optional)</label>
 <input
 value={ex.explanation}
 onChange={e => setArr('examples', i, 'explanation', e.target.value)}
 placeholder="nums[0] + nums[1] = 2 + 7 = 9, so we return [0, 1]"
 className="w-full mt-1 px-3 py-2 bg-bg-elevated border border-border-strong rounded-lg text-text-primary text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 placeholder-text-muted"
 />
 </div>
 </div>
 ))}
 ...
 <div key={i} className="bg-bg-surface/60 border border-border-default rounded-xl p-4 space-y-3">
 <div className="flex items-center justify-between">
 <span className="text-xs font-bold text-text-muted">Test Case {i + 1}</span>
 ...
 <div className="flex gap-2 text-[10px] text-text-muted pb-1">
 <span className="px-1.5 py-0.5 bg-bg-elevated rounded font-mono"># Approach</span>
 <span className="px-1.5 py-0.5 bg-bg-elevated rounded font-mono">**bold**</span>
 <span className="px-1.5 py-0.5 bg-bg-elevated rounded font-mono">`inline`</span>
 <span className="px-1.5 py-0.5 bg-bg-elevated rounded font-mono">``` code blocks</span>
 <span className="px-1.5 py-0.5 bg-bg-elevated rounded font-mono">- lists</span>
 </div>
 <textarea
 ...
 />
 <p className="text-[11px] text-text-muted">{form.editorial.length} chars</p>
 </div>
 </Field>
 </CollapsibleSection>
 ...
 <p className="text-[11px] text-text-muted"><span className="text-red-400">*</span> required</p>
 <div className="flex gap-3">
 <button type="button" onClick={onClose}
 className="px-5 py-2.5 rounded-xl text-sm font-semibold text-text-muted hover:text-text-primary bg-bg-elevated hover:bg-bg-overlay transition-all">
 Cancel
 </button>
