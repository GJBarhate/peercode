...
export default function VideoPanel({
  ...
  const renderRemotePeer = (peerId, peerStream) => {
    ...
    return (
      <div key={peerId} className={`relative rounded-xl overflow-hidden bg-gray-800 border transition-colors ${sharing ? 'border-green-600/40 shadow-lg shadow-green-900/20' : 'border-gray-700'} ${remoteEntries.length === 1 ? 'absolute inset-2' : 'w-full h-full'}`}>
        ...
        {state.isVideoOff && !sharing ? (
          <div className="flex flex-col items-center justify-center w-full h-full bg-gradient-to-b from-gray-900 to-gray-800">
            ...
            <span className="text-gray-500 text-xs mt-1">Camera is off</span>
          </div>
        ) : (
          <video
            key={vKey}
            ...
            autoPlay
            playsInline
            className={`w-full h-full ${sharing ? 'object-contain bg-gray-950' : 'object-cover'}`}
          />
        )}
      ...
      <div className="h-full flex flex-col">
        <div className="flex-1 relative flex items-center justify-center overflow-hidden p-2">
          {remoteEntries.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center w-full h-full rounded-xl bg-gradient-to-b from-gray-900/60 to-gray-900/30 border border-dashed border-gray-700/50">
              ...
              <p className="text-gray-400 text-sm font-medium">Waiting for partner...</p>
              <p className="text-gray-600 text-xs mt-1.5">Share the room link to invite someone</p>
            </div>
          )}
        </div>

        {localStream && (
          <div className="absolute bottom-16 right-3 z-10 w-32 h-24 rounded-xl overflow-hidden border-2 border-gray-700/50 shadow-xl bg-gray-800 transition-transform hover:scale-105">
            <VideoTile
