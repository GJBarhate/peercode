...
export default function VideoControls({ isMuted, isVideoOff, isSharing, onToggleMute, onToggleVideo, onHangUp, onScreenShare }) {
  ...
  return (
    <div className="flex items-center gap-2 p-1.5 bg-gray-900/95 backdrop-blur-md rounded-2xl border border-gray-800/80 shadow-xl shadow-black/30" role="toolbar" aria-label="Video controls">
      <button
        onClick={onToggleMute}
        className={`${btnBase} ${
          isMuted
            ? 'bg-red-600 hover:bg-red-500 text-white shadow-red-600/30 focus:ring-red-500'
            : 'bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white shadow-gray-800/50 focus:ring-indigo-400'
        }`}
      ...
      <div className="w-px h-7 bg-gray-700/50 mx-0.5" aria-hidden="true" />
