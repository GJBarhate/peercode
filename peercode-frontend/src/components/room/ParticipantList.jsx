...
export default function ParticipantList({ participants = [], currentUserId }) {
 ...
 <div
 key={uniqueKey}
 className="flex items-center gap-3 p-2.5 rounded-xl bg-bg-elevated/50 border border-border-strong/50 hover:bg-bg-elevated transition-colors"
 >
 ...
 <span className="text-xs text-text-muted capitalize">{p.role || 'interviewee'}</span>
 </div>
 ...
 <p className="text-xs text-text-muted text-center py-4">Waiting for participants...</p>
 )}
 </div>
 </div>
 )
