...
export default function RoomLobby({ roomId, onJoin, isLoading, error = null }) {
 ...
 <h1 className="text-3xl font-bold text-text-primary mb-2">Ready to practice?</h1>
 <p className="text-text-muted">Configure your setup before joining the room</p>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
 <div className="bg-bg-surface border border-border-default rounded-2xl p-6 space-y-4">
 <h2 className="font-semibold text-text-primary">Camera Preview</h2>
 <div className="relative bg-bg-elevated rounded-xl overflow-hidden aspect-video">
 ...
 <h2 className="font-semibold text-text-primary">Your Role</h2>
 ...
 : 'bg-bg-elevated/50 border-border-strong hover:border-border-strong'
 }`}
 >
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${
 role === value ? (color === 'amber' ? 'bg-amber-700' : 'bg-indigo-700') : 'bg-bg-overlay'
 }`}>
 ...
 role === value ? 'bg-indigo-500 border-indigo-400' : 'border-border-strong'
 }`} />
 </button>
 ))}
 </div>
