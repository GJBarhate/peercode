...
export default function InterviewTimer({ isInterviewer, onPhaseChange, onTimerEnd, socket, roomId }) {
 ...
 return (
 <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl bg-bg-surface border transition-colors ${
 isCriticalTime ? 'border-red-500/50' : isWarningTime ? 'border-amber-500/30' : 'border-border-default'
 }`}>
 <div className="relative w-8 h-8 flex-shrink-0">
 <svg viewBox="0 0 32 32" className="w-full h-full -rotate-90">
 <circle cx="16" cy="16" r="12" fill="none" stroke="var(--color-border-strong)" strokeWidth="3" />
 <circle
 cx="16" cy="16" r="12"
 fill="none"
 stroke={displayColor}
 ...
 isCriticalTime ? 'text-red-400' : isWarningTime ? 'text-amber-400' : 'text-text-primary'
 }`}>{formatTime(timeLeft)}</span>
 <span className={`text-[10px] capitalize transition-colors hidden sm:inline ${
 isCriticalTime ? 'text-red-400' : isWarningTime ? 'text-amber-400' : 'text-text-muted'
 }`}>
 {currentPhase.label}
 </span>
 </div>
 ...
 className="flex items-center gap-1 px-1.5 py-1 rounded text-[10px] font-medium bg-bg-elevated hover:bg-bg-overlay text-text-secondary border border-border-strong transition-colors"
 ...
 <div className="bg-bg-surface border border-border-strong rounded-2xl p-8 max-w-sm w-full mx-4 text-center shadow-2xl">
 <AlertCircle className="w-12 h-12 text-green-400 mx-auto mb-4" />
 <h3 className="text-xl font-bold text-text-primary mb-2">Time is up!</h3>
 <p className="text-text-muted text-sm mb-6">The session timer has completed.</p>
 <div className="flex gap-3 justify-center">
 <button
 onClick={() => setShowEndOverlay(false)}
 className="px-5 py-2 rounded-lg bg-bg-elevated hover:bg-bg-overlay text-text-primary font-semibold text-sm border border-border-strong transition-colors"
 >
 Continue
 </button>
 <button
