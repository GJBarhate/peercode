...
export default function SyntaxHighlight({ code, language = 'javascript' }) {
  ...
  return (
    <div className="relative group">
      <button
        onClick={handleCopy}
        className="absolute top-2 right-2 z-10 p-1.5 rounded-md bg-gray-800/80 hover:bg-gray-700 text-gray-400 hover:text-gray-200 opacity-0 group-hover:opacity-100 transition-opacity"
        title={copied ? 'Copied!' : 'Copy code'}
      >
        {copied ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
      </button>
      <pre className="bg-[#0a0a14] border border-gray-800 rounded-xl overflow-x-auto text-xs leading-relaxed font-mono">
        <code>
          {lines.map((line, li) => (
          <div key={li} className="flex hover:bg-white/[0.02]">
            <span className="flex-shrink-0 w-8 text-right pr-3 text-gray-700 select-none text-[11px]">{li + 1}</span>
