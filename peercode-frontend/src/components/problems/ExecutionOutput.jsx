...
export default function ExecutionOutput({ code, language, onClear }) {
 ...
 return (
 <div className="flex flex-col h-full bg-bg-base rounded-lg border border-border-default">
 ...
 <span className="text-sm font-semibold text-text-secondary">Output</span>
 </div>
 ...
 <button
 onClick={handleClear}
 className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-text-muted hover:text-text-primary hover:bg-bg-elevated transition-colors"
 >
 ...
 {!output && !error && logs.length === 0 ? (
 <p className="text-text-muted text-xs">No output yet. Click "Run" to execute code.</p>
 ) : error ? (
 ...
 {output && !logs.length && (
 <pre className="text-text-secondary whitespace-pre-wrap break-words">{output}</pre>
 )}
 </div>
 )}
 </div>
 ...
 <span className="text-xs font-semibold text-text-muted uppercase tracking-wider">Input/Paste</span>
 {input && (
 <button
 ...
 className="w-full bg-bg-elevated border border-border-strong text-text-primary rounded-lg px-3 py-2 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-950 placeholder-text-muted resize-none"
 rows={3}
 />
 </div>
 </div>
