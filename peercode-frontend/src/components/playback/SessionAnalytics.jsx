...
export default function SessionAnalytics({ roomId }) {
 ...
 <h4 className="text-sm font-semibold text-text-secondary mb-4">Code Growth Over Time</h4>
 ...
 <Tooltip
 contentStyle={{ backgroundColor: 'var(--color-chart-tooltip-bg)', border: '1px solid var(--color-chart-tooltip-border)', borderRadius: '8px', fontSize: '12px' }}
 labelStyle={{ color: 'var(--color-chart-tooltip-text)' }}
 itemStyle={{ color: 'var(--color-brand)' }}
 />
 {(data.pauseSegments || []).map((seg, i) => (
 <ReferenceLine key={i} x={seg.index} stroke="var(--color-error)" strokeDasharray="3 3" strokeOpacity={0.6} />
 ))}
 <Area
 type="monotone"
 dataKey="lines"
 ...
 <div className="bg-bg-surface border border-border-default rounded-xl p-5">
 <h4 className="text-sm font-semibold text-text-secondary mb-3">Activity Timeline</h4>
 ...
 style={{
 backgroundColor:
 seg === 'active' ? 'var(--color-success)' :
 seg === 'pause' ? 'var(--color-error)' :
 ...
 { color: 'bg-bg-elevated', label: 'Idle' }
 ].map(({ color, label }) => (
