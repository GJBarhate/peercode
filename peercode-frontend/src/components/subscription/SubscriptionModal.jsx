...
export default function SubscriptionModal({ isOpen, onClose, plan, orderData, onSuccess }) {
 ...
 <div className="relative w-full max-w-md transform rounded-2xl bg-bg-surface border border-border-default shadow-2xl transition-all">
 {step === 'loading' && (
 ...
 <p className="text-sm text-text-muted mt-1">Opening Razorpay checkout...</p>
 </div>
 ...
 Welcome to the <span className="font-semibold text-text-primary">{plan?.name}</span> plan
 </p>
 </div>
 ...
 <p className="text-sm text-text-muted mt-1">{errorMsg || 'Something went wrong.'}</p>
 </div>
 <div className="flex gap-3 justify-center">
 <button
 onClick={onClose}
 className="px-5 py-2.5 rounded-xl text-sm font-semibold bg-bg-elevated hover:bg-bg-overlay text-text-secondary transition-colors"
 >
 Close
 </button>
 </div>
