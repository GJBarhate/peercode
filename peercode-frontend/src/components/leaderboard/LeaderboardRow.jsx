...
const rankBg = {
  ...
  2: 'bg-gray-500/10 border-gray-500/20',
    ...
    <motion.div
      ...
      className={`flex items-center gap-4 px-4 py-3 rounded-xl border transition-colors ${
        isCurrentUser
          ...
          : isMedal
            ? `${rankBg[row.rank]}`
            : 'bg-gray-900/40 border-white/[0.04] hover:bg-gray-800/50'
      }`}
    >
      <div className={`w-8 text-center font-bold text-sm ${rankColors[row.rank] || 'text-gray-500'}`}>
        {row.rank <= 3 ? (
          <Trophy className={`w-4 h-4 mx-auto ${rankColors[row.rank] || 'text-gray-500'}`} />
        ) : (
          `#${row.rank}`
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className={`font-semibold text-sm truncate ${isCurrentUser ? 'text-indigo-300' : 'text-gray-200'}`}>
            {row.username}
          </span>
          {isCurrentUser && (
        ...
        <div className="text-xs text-gray-500">{row.totalMatches} matches · {row.winRate}% WR</div>
      </div>

      <div className="text-right">
        <div className="font-bold text-indigo-400">{row.elo}</div>
        <div className="text-xs text-gray-500">{row.wins}W</div>
      </div>
    </motion.div>
  )
})
