...
export default memo(function StatCard({ label, value, detail, icon: Icon, tone = 'indigo' }) {
  return (
    <div className="group bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 hover:-translate-y-1 hover:shadow-lg hover:shadow-indigo-500/5 hover:border-indigo-200 dark:hover:border-indigo-500/30 transition-all duration-200 cursor-default">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-500 truncate">{label}</p>
          <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-gray-100 tabular-nums">{value}</p>
          {detail && <p className="mt-1 text-xs text-gray-500 dark:text-gray-500 truncate">{detail}</p>}
        </div>
        {Icon && (
