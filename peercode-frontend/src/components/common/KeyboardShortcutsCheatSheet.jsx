...
export default function KeyboardShortcutsCheatSheet() {
 ...
 >
 <div
 className="bg-bg-surface border border-border-default rounded-2xl p-6 max-w-sm w-full shadow-xl"
 ...
 <div key={idx} className="flex items-center justify-between p-3 bg-bg-elevated/50 rounded-lg border border-border-strong/50">
 <span className="text-sm text-text-muted">{shortcut.action}</span>
 <div className="flex gap-1">
 {shortcut.keys.map((key, i) => (
 <span key={i} className="text-xs bg-bg-surface border border-border-strong rounded px-2 py-1 text-text-secondary font-mono">
 {key}
 </span>
 ))}
 </div>
