...
export default function RatingModal({ isOpen, onClose, sessionId, partnerName, partnerId }) {
 ...
 onClick={onClose}
 >
 <div className="bg-bg-surface border border-border-default rounded-2xl p-6 w-full max-w-sm mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
 <div className="flex items-center justify-between mb-4">
 <h3 id="rating-modal-title" className="text-lg font-bold text-text-primary">Rate Your Partner</h3>
 <button onClick={onClose} aria-label="Close rating modal" className="p-1 rounded-lg hover:bg-bg-elevated text-text-muted hover:text-text-secondary transition-colors">
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 How was your session with <span className="font-semibold text-text-primary">{partnerName}</span>?
 </p>
 )}
 ...
 star <= (hovered || score) ? 'text-amber-400 fill-amber-400' : 'text-text-muted'
 }`}
 />
 </button>
 ))}
 ...
 rows={3}
 maxLength={500}
 className="w-full px-3 py-2 bg-bg-elevated border border-border-strong rounded-xl text-sm text-text-primary placeholder-text-muted resize-none focus:outline-none focus:ring-1 focus:ring-indigo-500"
 />
