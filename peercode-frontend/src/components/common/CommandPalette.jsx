      ...
      <div className="fixed inset-0 z-[9998] flex items-start justify-center pt-[15vh] px-4" onKeyDown={handleKey}>
        ...
        <div className="relative w-full max-w-lg glass-card gradient-border rounded-2xl shadow-2xl overflow-hidden">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-white/[0.06]">
            ...
            />
            <kbd className="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-white/[0.06] border border-white/[0.1] text-[10px] text-gray-500 font-mono">
              ESC
            </kbd>
          </div>
          ...
          <div id="command-palette-listbox" role="listbox" aria-label="Commands" className="max-h-[50vh] overflow-y-auto py-2">
            {results.length === 0 ? (
              ...
              Object.entries(grouped).map(([group, cmds]) => (
                <div key={group}>
                  <div className="px-4 pt-2 pb-1 text-[10px] font-bold uppercase tracking-widest text-gray-500">{group}</div>
                  {cmds.map(cmd => {
                    globalIdx++
                    const idx = globalIdx
                    const Icon = cmd.icon
          ...
          <div className="px-4 py-2 border-t border-white/[0.06] flex items-center gap-4 text-[10px] text-gray-600">
