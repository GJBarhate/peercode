...
export default function AIHintPanel({ code, language, problem }) {
 ...
 className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium bg-bg-elevated hover:bg-bg-overlay text-text-secondary border border-border-strong transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
 >
 ...
 <button
 onClick={handleGetHint}
 className="ml-auto p-1 text-text-muted hover:text-text-secondary transition-colors flex-shrink-0"
 title="Get another hint"
 >
 <RefreshCw className="w-3 h-3" />
 </button>
 </div>
 <p className="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1">{hint}</p>
 </div>
 )}

 {analysis && (
 <div className="mx-4 mb-4 border border-border-strong rounded-xl overflow-hidden max-h-full flex flex-col">
 <button
 onClick={() => setAnalysisExpanded(v => !v)}
 className="w-full flex items-center justify-between px-4 py-3 bg-bg-elevated hover:bg-bg-overlay transition-colors flex-shrink-0"
 >
 <div className="flex items-center gap-2">
 <Code2 className="w-4 h-4 text-green-400" />
 <span className="text-sm font-semibold text-text-primary">Code Analysis</span>
 </div>
 {analysisExpanded ? <ChevronUp className="w-4 h-4 text-text-muted" /> : <ChevronDown className="w-4 h-4 text-text-muted" />}
 </button>
 {analysisExpanded && (
 <div className="p-4 space-y-4 bg-bg-surface overflow-y-auto max-h-80">
 ...
 <p className="text-sm text-amber-400 font-mono mt-1.5 bg-bg-elevated/50 p-2 rounded">{analysis.timeComplexity}</p>
 </div>
 )}
 {analysis.spaceComplexity && (
 <div className="pb-3 border-b border-border-default">
 <span className="text-xs font-semibold text-blue-500 uppercase tracking-wider">💾 Space Complexity</span>
 <p className="text-sm text-blue-400 font-mono mt-1.5 bg-bg-elevated/50 p-2 rounded">{analysis.spaceComplexity}</p>
 </div>
 )}
 {analysis.feedback && (
 <div>
 <span className="text-xs font-semibold text-green-500 uppercase tracking-wider">📋 Detailed Feedback</span>
 <p className="text-sm text-text-secondary mt-2 leading-relaxed whitespace-pre-wrap">{analysis.feedback}</p>
 </div>
 )}
 ...
 <li key={i} className="text-sm text-text-muted flex gap-2 bg-indigo-900/20 p-2 rounded border border-indigo-800/30">
