...
export default function ProblemDetailPage() {
 ...
 const contestSlug = searchParams.get('contest') || null
 ...
 setProblem(data)
 setCode(data.starterCode?.[language] || data.stubs?.[language] || PROBLEM_STARTER_CODE[data.slug]?.[language] || STARTER_CODE[language] || '// Start coding here\n')
 } catch (err) {
 ...
 PROBLEM_STARTER_CODE[problem.slug]?.[language] ||
 ...
 problem?.stubs?.[lang] ||
 PROBLEM_STARTER_CODE[problem?.slug]?.[lang] ||
 STARTER_CODE[lang] ||
 '// Start coding here\n'
 )
 }
 ...
 <div className="h-14 bg-bg-surface border-b border-border-default px-5 flex items-center justify-between flex-shrink-0">
 <div className="flex items-center gap-2">
 <button
 onClick={() => navigate(contestSlug ? `/contests/${contestSlug}` : '/problems')}
 className="btn-ghost text-sm"
 >
 <ArrowLeft className="w-4 h-4" />
 </button>
 <span className="text-text-muted">/</span>
 <span className="text-sm text-text-primary font-medium truncate max-w-xs">{problem?.title}</span>
 {contestSlug && (
 <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-gradient-to-r from-amber-500/15 to-orange-500/15 border border-amber-500/30 text-amber-500 text-xs font-semibold flex-shrink-0">
 🏆 Contest Mode
 </span>
 )}
 </div>
 ...
 <div 
 className={`border-r border-border-default bg-bg-surface transition-all duration-200 overflow-hidden flex flex-col ${
 ...
 className="w-1.5 bg-transparent hover:bg-accent/40 cursor-col-resize transition-colors flex-shrink-0"
 />
 )}
 ...
 className="absolute left-4 top-20 z-10 p-1 hover:bg-bg-hover rounded transition-colors"
 ...
 className="absolute left-0 top-20 z-10 p-1 hover:bg-bg-hover rounded transition-colors"
 ...
 {/* Editor Section */}
 <div className="flex-1 flex flex-col overflow-hidden border-b border-border-default">
 <EditorToolbar
 language={language}
 ...
 {/* Test Panel Section */}
 <div
 className="test-panel-container bg-bg-surface overflow-hidden flex flex-col flex-shrink-0"
 style={{ height: `${testPanelHeight}px` }}
 >
 <div
 onMouseDown={() => setIsDraggingV(true)}
 className="h-1.5 bg-transparent hover:bg-accent/40 cursor-row-resize transition-colors flex-shrink-0"
 />
 <TestCaseRunner
 code={code}
 language={language}
 ...
 {/* AI Sidebar */}
 {aiMode && (
 <div className="w-96 border-l border-border-default bg-bg-surface flex flex-col overflow-hidden flex-shrink-0">
 {/* Header */}
 ...
 <button onClick={handleCloseAi} className="p-1 rounded-lg hover:bg-bg-elevated text-text-muted hover:text-text-secondary transition-colors">
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 <p className="text-sm text-text-muted">Generating {aiMode === 'hint' ? 'hint' : 'analysis'}...</p>
 </div>
 )}
 ...
 <p className="text-sm text-text-primary leading-relaxed whitespace-pre-wrap">{hint}</p>
 </div>
 <button
 ...
 <p className="text-sm text-text-primary font-mono">{analysis.timeComplexity || 'N/A'}</p>
 </div>
 ...
 <p className="text-sm text-text-primary font-mono">{analysis.spaceComplexity || 'N/A'}</p>
 </div>
 </div>

 {analysis.feedback && (
 <div className="space-y-2">
 <h4 className="text-xs font-semibold text-text-muted uppercase tracking-wider">Feedback</h4>
 <div className="p-4 rounded-xl bg-bg-elevated/50 border border-border-strong/50">
 <p className="text-sm text-text-secondary leading-relaxed">{analysis.feedback}</p>
 </div>
 </div>
 )}
 ...
 <li key={i} className="flex items-start gap-2.5 p-3 rounded-lg bg-bg-elevated/30 border border-border-strong/30">
