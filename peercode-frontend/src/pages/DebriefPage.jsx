    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-40 h-40">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-xs text-[#5a5a72] font-medium mt-0.5">/100</span>
        </div>
      </div>
...
function MetricBar({ label, value, color = '#6d4df2', icon: Icon }) {
  return (
    <div className="group p-4 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] border border-white/[0.04] hover:border-white/[0.08] transition-all duration-200">
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          {Icon && <Icon className="w-4 h-4" style={{ color }} />}
          <span className="text-sm text-[#f1f1f5] font-medium">{label}</span>
        </div>
        <span className="text-sm font-bold tabular-nums" style={{ color }}>{value}<span className="text-[#5a5a72] font-normal text-xs">/100</span></span>
      </div>
...
function ScoreCard({ label, value, max, color, icon: Icon }) {
  ...
  return (
    <div className="group flex flex-col items-center gap-2 p-4 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] border border-white/[0.04] hover:border-white/[0.08] transition-all duration-200 cursor-default">
      ...
      <div className="text-center">
        <div className="text-2xl font-black" style={{ color }}>{value}<span className="text-xs text-[#5a5a72] font-normal">/{max}</span></div>
        <div className="text-[11px] text-[#5a5a72] font-medium mt-0.5">{label}</div>
      </div>
    ...
    try {
      ...
      if (err.response?.status === 404) {
        try {
          ...
          if (sessionId) await generateDebrief(sessionId)
          const participants = sessionData?.participants || []
          const partnerId = participants.find(id => String(id) !== String(user?._id || user?.id))
          if (partnerId) setPartner({ _id: partnerId })
        } catch (_) {}
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#0a0a14]">
        <Navbar />
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 pb-16 space-y-5">
      ...
      <div className="min-h-screen bg-[#0a0a14]">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 pb-16">
          <div className="flex flex-col items-center justify-center min-h-[55vh] gap-6">
            ...
            <div className="text-center">
              <h2 className="text-2xl font-bold text-gray-200 mb-2">Analyzing Your Session</h2>
              ...
              {isRetrying && (
                <p className="text-xs text-gray-600 mt-3 tabular-nums">
            ...
            <button onClick={load} disabled={isLoading} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-indigo-400 hover:text-white bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 hover:border-indigo-500/50 transition-all disabled:opacity-50 active:scale-[0.98]">
      ...
      <div className="min-h-screen bg-[#0a0a14]">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 pb-16">
          <div className="flex flex-col items-center justify-center min-h-[55vh] gap-6">
            ...
            <div className="text-center">
              <h2 className="text-2xl font-bold text-gray-200 mb-2">Failed to Load Debrief</h2>
              <p className="text-sm text-gray-500 max-w-sm">{error}</p>
            </div>
            <button onClick={load} disabled={isLoading} className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 transition-all active:scale-[0.98]">
              <RotateCcw className={`w-4 h-4 ${isLoading ? 'animate-spin' : ''}`} />
  ...
  return (
    <div className="min-h-screen bg-[#0a0a14]">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 pb-16 space-y-5">
        ...
        {/* Hero — Problem + Score */}
        <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-gradient-to-br from-[#11111f] via-[#0f0f1e] to-[#1a1a2e] p-6 sm:p-8">
        ...
        {/* Tab Switcher */}
        <div className="flex gap-1 p-1 bg-[#11111f] border border-white/[0.06] rounded-xl">
          {[
        ...
        {/* Summary */}
        {debrief.summary && (
          <div className="relative bg-[#11111f] border border-indigo-500/20 rounded-xl p-5 pl-6">
        ...
        {hasScores && (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-sm font-bold text-white uppercase tracking-wider">Performance Breakdown</h2>
            </div>
        ...
        {/* Progress Bars */}
        {hasScores && (
          <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5 space-y-2">
            <div className="flex items-center gap-2 mb-4">
              <Cpu className="w-4 h-4 text-indigo-400" />
              <h2 className="text-sm font-bold text-white uppercase tracking-wider">Skill Metrics</h2>
            </div>
              ...
              <ul className="space-y-3">
                {whatWentWell.map((s, i) => (
                  <li key={i} className="flex items-start gap-3 group">
                    ...
                    <span className="text-sm text-gray-400 leading-relaxed group-hover:text-gray-300 transition-colors">{s}</span>
                  </li>
                ))}
              </ul>
            ) : (
          ...
          <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5">
            ...
            {areasToImprove.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 gap-2 text-center">
                ...
                <p className="text-sm text-gray-600">No specific improvements recorded.</p>
              </div>
            )}
          </div>
        </div>
        ...
        {debrief.weakTopics?.length > 0 && (
          <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-bold text-white uppercase tracking-wider">Focus Areas</h2>
            </div>
        ...
        {partner && !ratingSubmitted && (
          <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-bold text-white uppercase tracking-wider">Rate Your Partner</h2>
            </div>
            <p className="text-sm text-gray-500 mb-4">How was your collaboration experience?</p>
            ...
            <textarea
              ...
              rows={2}
              maxLength={500}
              className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-200 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500/50 placeholder-gray-600 resize-none mb-3 transition-colors"
            />
            <button
              onClick={async () => {
                ...
                try {
                  await submitPartnerRating({ sessionId: debrief.sessionId || debrief._id, toUserId: partner._id, score: rating, feedback: ratingFeedback })
              ...
              className="flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold text-white bg-amber-500 hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-[0.98]"
            >
          ...
          {(debrief.timeComplexity || debrief.spaceComplexity) && (
            <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Complexity Analysis</h2>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                {debrief.timeComplexity && (
                  <div className="p-4 rounded-xl bg-cyan-500/5 border border-cyan-500/20">
                    ...
                    <p className="text-sm text-gray-300">{debrief.timeComplexity}</p>
                  </div>
                )}
                {debrief.spaceComplexity && (
                  <div className="p-4 rounded-xl bg-violet-500/5 border border-violet-500/20">
                    ...
                    <p className="text-sm text-gray-300">{debrief.spaceComplexity}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {debrief.approachAnalysis && (
            <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Approach Analysis</h2>
              </div>
              <p className="text-sm text-gray-300 leading-relaxed">{debrief.approachAnalysis}</p>
            </div>
          )}

          {/* Interviewer Perspective */}
          {debrief.interviewerPerspective && (
            <div className="bg-[#11111f] border border-amber-500/20 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                <Eye className="w-4 h-4 text-amber-400" />
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Interviewer&apos;s Perspective</h2>
              </div>
              <div className="relative pl-4 border-l-2 border-amber-500/40">
                <p className="text-sm text-gray-300 leading-relaxed italic">&ldquo;{debrief.interviewerPerspective}&rdquo;</p>
              </div>
            </div>
          )}
          ...
          {debrief.similarProblems?.length > 0 && (
            <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Similar Problems to Practice</h2>
              </div>
              <div className="space-y-3">
                {debrief.similarProblems.map((p, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.04] transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm text-gray-200 truncate">{p.title}</div>
          ...
          {debrief.improvementPlan?.length > 0 ? (
            <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5">
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Your Personalized Improvement Plan</h2>
              </div>
              <div className="space-y-4">
                {debrief.improvementPlan.map((step, i) => (
                  <div key={i} className="flex gap-4 group">
                    <div className="flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-indigo-600 to-violet-600 flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-indigo-600/20">
                      {i + 1}
                    </div>
                    <div className="flex-1 pt-1">
                      <p className="text-sm text-gray-300 leading-relaxed group-hover:text-gray-100 transition-colors">{step}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-8 text-center">
              <Lightbulb className="w-10 h-10 text-gray-600 mx-auto mb-3" />
              <p className="text-gray-500 text-sm">No improvement plan available for this session.</p>
            </div>
          )}
          ...
          {debrief.studyNext?.length > 0 && (
            <div className="bg-[#11111f] border border-white/[0.06] rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Topics to Study Next</h2>
              </div>
              <div className="grid gap-3">
                {debrief.studyNext.map((tip, i) => (
                  <div key={i} className="group flex items-start gap-3 bg-white/[0.02] hover:bg-white/[0.04] rounded-xl p-4 border border-white/[0.04] transition-all">
                    ...
                    <p className="text-sm text-gray-400 leading-relaxed pt-1 group-hover:text-gray-300 transition-colors">{tip}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 pt-2 flex-wrap">
          {debrief.problemSlug && (
            <Link
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 shadow-lg shadow-indigo-600/20 hover:shadow-indigo-600/30 transition-all active:scale-[0.98]"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-semibold text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 transition-all active:scale-[0.98]"
          >
