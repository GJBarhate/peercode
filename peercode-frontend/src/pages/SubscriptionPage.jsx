...
export default function SubscriptionPage() {
 ...
 <main className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-6 pb-24">
 {/* Header */}
 ...
 <div className="inline-flex items-center gap-2 p-1 bg-bg-surface border border-border-default rounded-xl">
 <button
 onClick={() => setBilling('monthly')}
 className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${billing === 'monthly' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-text-muted hover:text-text-secondary'}`}
 >
 Monthly
 </button>
 <button
 onClick={() => setBilling('yearly')}
 className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${billing === 'yearly' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-text-muted hover:text-text-secondary'}`}
 >
 Yearly
 ...
 : 'border-border-default bg-bg-surface/80 hover:border-border-strong'
 }`}
 >
 {/* Badges */}
 {isPopular && !isCurrent && (
 ...
 {plan.limit && <p className="text-xs text-text-muted">Up to {plan.limit} hints</p>}
 </div>
 </div>
 ...
 <span className="text-text-muted">/{billing === 'yearly' ? 'yr' : 'mo'}</span>
 </div>
 ...
 <span className="text-sm text-text-muted line-through">₹{billing === 'yearly' ? Math.round(plan.originalPrice * 12 * 0.8) : plan.originalPrice}</span>
 ...
 plan.id === 'ultra' ? 'text-pink-400' : 'text-text-muted'
 }`} />
 ...
 <div className="mb-4 p-3 bg-bg-surface/5 dark:bg-bg-surface/5 rounded-xl">
 <p className="text-xs text-text-muted mb-2 font-medium">Your Usage</p>
 <div className="space-y-2">
 <div>
 <div className="flex justify-between text-xs">
 <span className="text-text-muted">Hints</span>
 <span className="text-text-secondary">{status.usage.hints.used}/{status.usage.hints.limit === Infinity ? '∞' : status.usage.hints.limit}</span>
 </div>
 ...
 <span className="text-text-secondary">{status.usage.analyzes.used}/{status.usage.analyzes.limit === Infinity ? '∞' : status.usage.analyzes.limit}</span>
 </div>
 ...
 {/* CTA Button */}
 {plan.id === 'free' ? (
 <Link to="/profile" className="block w-full py-3.5 rounded-xl text-center text-sm font-bold bg-bg-elevated hover:bg-bg-overlay text-text-secondary border border-border-strong transition-colors">
 Free - Always
 </Link>
 ) : isCurrent ? (
 ...
 : 'bg-bg-elevated hover:bg-bg-overlay text-text-secondary border border-border-strong'
 }`}
 >
 {currentPlan === 'free' ? (
 ...
 {plans.map(p => (
 <th key={p.id} className="text-center py-4 px-4 text-text-secondary font-semibold">{p.name}</th>
 ))}
 </tr>
 </thead>
 <tbody>
 ...
 return (
 <tr key={feat} className="border-b border-border-default/50 hover:bg-white/[0.02]">
 <td className="py-3.5 px-4 text-text-muted font-medium">{feat}</td>
 {values[feat].map((v, j) => (
 <td key={j} className={`text-center py-3.5 px-4 ${
 v === '✓' ? 'text-green-600 dark:text-green-400' : v === '∞' ? 'text-pink-400 font-bold' : 'text-text-muted'
 }`}>
 {v}
 </td>
 ))}
 ...
 <details key={i} className="group bg-bg-surface border border-border-default rounded-xl overflow-hidden">
 <summary className="flex items-center justify-between p-4 cursor-pointer text-text-secondary font-medium hover:bg-white/[0.02] transition-colors">
 {faq.q}
