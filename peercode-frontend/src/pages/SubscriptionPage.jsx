...
import { Check, Crown, Gem, Shield, ArrowLeft, Loader2, Sparkles, Star, TrendingUp, Zap, AlertCircle, CheckCircle2 } from 'lucide-react'
import { Link, useNavigate } from 'react-router-dom'
import Navbar from '../components/common/Navbar'
import Skeleton from '../components/common/Skeleton'
import { getPlans, createSubscription, getSubscriptionStatus, cancelSubscription, getProfile } from '../services/api'
...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (plansRes.data?.plans) {
          setPlans(plansRes.data.plans.map((p, i) => ({ ...PLANS[i], ...p })))
        }
        setStatus(statusRes.data || { plan: 'free', status: 'active' })
      } catch (err) {
    ...
    // Handle mock payment success return
    const params = new URLSearchParams(window.location.search)
    if (params.get('payment') === 'success' && params.get('mock') === 'true') {
      toast.success('Payment successful (dev mode)!', { duration: 3000 })
      params.delete('payment')
      params.delete('mock')
      params.delete('plan')
      window.history.replaceState({}, '', `${window.location.pathname}${params.toString() ? '?' + params.toString() : ''}`)
      // Refresh status after mock payment
      setTimeout(() => load(), 500)
    }
  }, [])
  ...
  const handleSubscribe = async (planId) => {
    ...
    try {
      ...
      if (data.shortUrl) {
        toast.success(data.mock ? 'Opening dev payment page...' : 'Opening payment page...', { duration: 3000 })
        const opened = window.open(data.shortUrl, '_blank', 'noopener,noreferrer')
        if (!opened) {
      ...
      <div className="relative overflow-hidden">
        ...
        <main className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-20 pb-24">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-5xl font-black text-gray-100 mb-4">
              Choose Your{' '}
            ...
            <div className="max-w-3xl mx-auto mb-10">
              <div className="bg-gradient-to-r from-indigo-900/40 to-purple-900/40 border border-indigo-500/30 rounded-2xl p-6">
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={handleCancel} className="px-4 py-2 rounded-lg border border-red-800/50 text-red-400 hover:bg-red-900/30 text-sm font-medium transition-colors">
                    Cancel
                  </button>
                </div>
              </div>
          ...
          <div className="flex justify-center mb-10">
            <div className="inline-flex items-center gap-2 p-1 bg-gray-900 border border-gray-800 rounded-xl">
              <button
                ...
                className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${billing === 'monthly' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-gray-400 hover:text-gray-200'}`}
              >
                Monthly
              </button>
              <button
                onClick={() => setBilling('yearly')}
                className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${billing === 'yearly' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-gray-400 hover:text-gray-200'}`}
              >
                Yearly
                <span className="ml-1.5 text-[10px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded-full">-20%</span>
              </button>
            </div>
          </div>
              ...
              return (
                <div
                  ...
                  className={`relative flex flex-col rounded-2xl border-2 transition-all duration-300 hover:scale-[1.02] ${
                    isCurrent 
                      ? 'border-indigo-500 bg-gradient-to-b from-indigo-900/30 to-gray-900 shadow-xl shadow-indigo-500/10'
                      : isPopular
                        ? 'border-amber-500/50 bg-gradient-to-b from-amber-900/10 to-gray-900 shadow-lg'
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <div className="mb-6">
                      ...
                      {billing === 'yearly' && plan.price > 0 && (
                        <p className="text-xs text-green-400 mt-1">₹{plan.price}/mo billed annually (save 20%)</p>
                      )}
                      {plan.originalPrice && (
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full font-medium">Save {Math.round((1 - plan.price / plan.originalPrice) * 100)}%</span>
                        </div>
                      )}
                    </div>
                    ...
                    {isCurrent && status?.usage && (
                      <div className="mb-4 p-3 bg-white/5 rounded-xl">
                        <p className="text-xs text-gray-400 mb-2 font-medium">Your Usage</p>
                        <div className="space-y-2">
                          <div>
                            <div className="flex justify-between text-xs">
                              <span className="text-gray-500">Hints</span>
                              <span className="text-gray-300">{status.usage.hints.used}/{status.usage.hints.limit === Infinity ? '∞' : status.usage.hints.limit}</span>
                            </div>
                          ...
                          <div>
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="text-gray-300">{status.usage.analyzes.used}/{status.usage.analyzes.limit === Infinity ? '∞' : status.usage.analyzes.limit}</span>
                            </div>
                    ...
                    {/* CTA Button */}
                    {plan.id === 'free' ? (
                      <Link to="/profile" className="block w-full py-3.5 rounded-xl text-center text-sm font-bold bg-gray-800 hover:bg-gray-700 text-gray-300 border border-gray-700 transition-colors">
                        Free - Always
                      </Link>
                    ) : isCurrent ? (
                        ...
                        {showConfirm === plan.id ? (
                          <div className="space-y-2">
                            <button
                              ...
                              className="w-full py-3.5 rounded-xl text-sm font-bold bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 text-white transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                            >
                              {subscribing === plan.id ? (
                                <><Loader2 className="w-4 h-4 animate-spin" /> Processing...</>
                              ) : (
                                <><CheckCircle2 className="w-4 h-4" /> Confirm - ₹{billing === 'yearly' ? yearlyPrice : plan.price}</>
                              )}
                            </button>
                            <button
          ...
          <div className="mt-16 max-w-3xl mx-auto">
            <div className="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border border-indigo-800/30 rounded-2xl p-8 text-center">
          ...
          {/* Comparison Table */}
          <div className="mt-16">
            <h2 className="text-2xl font-bold text-gray-100 text-center mb-8">Compare Plans</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    ...
                    {plans.map(p => (
                      <th key={p.id} className="text-center py-4 px-4 text-gray-300 font-semibold">{p.name}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {['Hints per month', 'Analyzes per month', 'API pool', 'Support', 'Advanced analytics', 'Session export', 'Custom AI', 'API access'].map((feat, i) => {
                    ...
                    return (
                      <tr key={feat} className="border-b border-gray-800/50 hover:bg-white/[0.02]">
                        <td className="py-3.5 px-4 text-gray-400 font-medium">{feat}</td>
                        {values[feat].map((v, j) => (
                          <td key={j} className={`text-center py-3.5 px-4 ${
                            v === '✓' ? 'text-green-400' : v === '∞' ? 'text-pink-400 font-bold' : 'text-gray-500'
                          }`}>
                            {v}
                          </td>
                        ))}
          ...
          <div className="mt-16 max-w-2xl mx-auto">
            <h2 className="text-2xl font-bold text-gray-100 text-center mb-8">Frequently Asked Questions</h2>
            <div className="space-y-4">
              {[
                ...
                <details key={i} className="group bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  <summary className="flex items-center justify-between p-4 cursor-pointer text-gray-200 font-medium hover:bg-white/[0.02] transition-colors">
                    {faq.q}
