...
import { useOnboardingTour, interviewSteps } from '../hooks/useOnboardingTour'
...
function AIAvatar({ isSpeaking, isListening, isThinking }) {
 ...
 idle: 'bg-bg-elevated/60 text-text-muted',
 }[state]
...
function SetupPhase({ config, setConfig, onStart, isLoading }) {
 ...
 <div>
 <label className="block text-sm font-semibold text-text-secondary mb-2">Resume <span className="text-text-muted font-normal">(optional)</span></label>
 <div onClick={() => fileRef.current?.click()}
 className="border-2 border-dashed border-border-strong hover:border-indigo-500/50 rounded-xl p-6 text-center cursor-pointer transition-all group hover:bg-indigo-500/5">
 <input ref={fileRef} type="file" accept=".txt,.pdf,.doc,.docx" onChange={handleFile} className="hidden"/>
 {config.resume
 ? <div className="flex items-center justify-center gap-3"><CheckCircle2 className="w-7 h-7 text-green-400"/><div><p className="text-green-400 font-semibold text-sm">Uploaded</p><p className="text-[11px] text-text-muted">{config.resume.length} chars</p></div></div>
 : <><Upload className="w-8 h-8 text-text-muted mx-auto mb-2 group-hover:text-indigo-400 transition-colors"/><p className="text-text-muted text-sm">Click to upload or paste below</p></>}
 </div>
 <textarea value={config.resume || ''} onChange={e => setConfig(p => ({ ...p, resume: e.target.value }))}
 placeholder="Or paste resume text here…" rows={3}
 className="mt-2 w-full bg-bg-surface/60 border border-border-default rounded-xl px-4 py-3 text-sm text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-indigo-500/40 resize-none"/>
 </div>
 <div>
 <label className="block text-sm font-semibold text-text-secondary mb-2">Job Description <span className="text-text-muted font-normal">(optional)</span></label>
 ...
 className="w-full bg-bg-surface/60 border border-border-default rounded-xl px-4 py-3 text-sm text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-indigo-500/40 resize-none"/>
 </div>
 </div>,
 ...
 <div>
 <label className="block text-sm font-semibold text-text-secondary mb-3">Target Company</label>
 ...
 className={`p-2.5 rounded-xl border text-xs font-medium transition-all ${chosen ? 'border-indigo-500 bg-indigo-500/10 text-indigo-300 ring-1 ring-indigo-500/25' : 'border-border-default text-text-muted hover:border-border-strong hover:bg-bg-surface/50'}`}>
 ...
 <div>
 <label className="block text-sm font-semibold text-text-secondary mb-3">Interview Type</label>
 <div className="grid grid-cols-2 gap-2">
 {INTERVIEW_TYPES.map(t => (
 <button key={t.id} onClick={() => setConfig(p => ({ ...p, interviewType: t.id }))}
 className={`p-3 rounded-xl border text-left transition-all ${config.interviewType === t.id ? 'border-indigo-500 bg-indigo-500/10 ring-1 ring-indigo-500/25' : 'border-border-default hover:border-border-strong hover:bg-bg-surface/40'}`}>
 ...
 <span className="font-semibold text-text-primary text-xs">{t.label}</span>
 </div>
 <p className="text-[10px] text-text-muted ml-9">{t.desc}</p>
 </button>
 ))}
 </div>
 </div>
 ...
 <div>
 <label className="block text-sm font-semibold text-text-secondary mb-3">Difficulty</label>
 <div className="grid grid-cols-3 gap-3">
 {DIFFICULTIES.map(d => (
 <button key={d.id} onClick={() => setConfig(p => ({ ...p, difficulty: d.id }))}
 className={`p-4 rounded-xl border text-center transition-all ${config.difficulty === d.id ? d.cls + ' ring-1' : 'border-border-default text-text-muted hover:border-border-strong'}`}>
 ...
 <div>
 <label className="block text-sm font-semibold text-text-secondary mb-3">Interview Duration</label>
 <div className="grid grid-cols-5 gap-2">
 {DURATIONS.map(d => (
 <button key={d.mins} onClick={() => setConfig(p => ({ ...p, duration: d.mins }))}
 className={`p-3 rounded-xl border text-center transition-all ${config.duration === d.mins ? 'border-indigo-500 bg-indigo-500/10 text-indigo-300 ring-1 ring-indigo-500/25' : 'border-border-default text-text-muted hover:border-border-strong'}`}>
 ...
 <div className="bg-bg-surface/50 border border-border-default rounded-xl p-4">
 <p className="text-xs font-semibold text-text-muted mb-2 uppercase tracking-wider">Summary</p>
 ...
 <div key={k}><span className="text-text-muted">{k}:</span><span className="text-text-primary ml-1.5 capitalize">{v}</span></div>
 ))}
 </div>
 </div>
 </div>,
 ...
 return (
 <div data-tour="interview-setup" className="max-w-xl mx-auto">
 {/* progress dots */}
 ...
 <div className={`h-full transition-all duration-500 ${i <= step ? 'bg-indigo-500' : 'bg-bg-elevated'}`} />
 </div>
 )}
 ...
 : 'bg-bg-surface border-border-strong text-text-muted'
 }`}>
 {i < step ? '✓' : i + 1}
 </div>
 <p className={`text-[10px] font-medium text-center transition-colors ${i <= step ? 'text-indigo-400' : 'text-text-muted'}`}>{lbl}</p>
 </div>
 ))}
 </div>
 ...
 className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm text-text-muted hover:text-text-primary disabled:opacity-25 transition-colors">
...
function InterviewPhase({ questions, config, onComplete, speech }) {
 ...
 <div className={`flex items-center gap-2 px-3.5 py-2 rounded-xl font-mono font-bold text-sm transition-all ${lowGlobal ? 'bg-red-500/20 text-red-400 ring-1 ring-red-500/20' : 'bg-bg-surface text-text-primary border border-border-default'}`}>
 ...
 {questions.map((_, i) => (
 <div key={i} className={`rounded-full transition-all duration-300 ${i < qi ? 'w-5 h-2 bg-indigo-500' : i === qi ? 'w-8 h-2 bg-indigo-400' : 'w-2 h-2 bg-bg-elevated'}`}/>
 ))}
 </div>
 ...
 <span className={`text-[10px] font-bold font-mono tabular-nums z-10 ${lowPer ? 'text-red-400' : 'text-text-secondary'}`}>
 {pM}:{pS.toString().padStart(2,'0')}
 </span>
 </div>
 <span className="text-xs text-text-muted">Q{qi+1}/{questions.length}</span>
 </div>
 </div>
 ...
 className={`mt-4 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${voiceEnabled ? 'bg-indigo-500/15 text-indigo-300' : 'bg-bg-elevated text-text-muted'}`}>
 ...
 {/* question card */}
 <div className="bg-bg-surface/70 border border-border-default/80 rounded-2xl p-5">
 ...
 {q?.expectedTopics?.slice(0,3).map((t,i) => <span key={i} className="px-1.5 py-0.5 bg-bg-elevated text-text-muted rounded text-[10px] hidden sm:inline">{t}</span>)}
 </div>
 <p className="text-text-primary text-sm leading-relaxed">{q?.question}</p>
 </div>

 {/* answer area */}
 <div className="bg-bg-surface/50 border border-border-default/80 rounded-2xl p-5 min-h-[180px] relative flex flex-col">
 {useType
 ? <textarea value={typed} onChange={e => setTyped(e.target.value)} placeholder="Type your answer here…" autoFocus
 className="flex-1 bg-transparent text-text-primary text-sm leading-relaxed placeholder-text-muted focus:outline-none resize-none"/>
 ...
 : <><Mic className="w-9 h-9 text-text-secondary mb-2"/><p className="text-text-muted text-sm">Press mic to start speaking</p></>}
 </div>}
 </div>}
 <button onClick={() => { setUseType(u => !u); if (isListening) stopListening() }}
 className="absolute top-3.5 right-3.5 text-[11px] px-2.5 py-1 rounded-lg bg-bg-elevated text-text-muted hover:text-text-secondary transition-colors">
 {useType ? '🎤 Voice' : '⌨ Type'}
 </button>
 </div>
 ...
 {!isLast && (
 <button onClick={skip} className="px-4 py-3.5 rounded-2xl text-sm text-text-muted hover:text-text-secondary border border-border-default hover:border-border-strong transition-all">
 Skip
 </button>
 )}
 </div>
...
function FeedbackPhase({ feedback, onRestart }) {
 ...
 {/* score card */}
 <div className="relative overflow-hidden bg-bg-surface border border-border-default rounded-2xl p-8 text-center">
 <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(99,102,241,0.06),transparent_65%)] pointer-events-none"/>
 <h2 className="text-base font-bold text-text-secondary mb-6">Interview Complete</h2>
 <div className="relative w-32 h-32 mx-auto mb-5">
 <svg className="w-32 h-32 -rotate-90" viewBox="0 0 116 116">
 <circle cx="58" cy="58" r="52" fill="none" stroke="var(--color-border-strong)" strokeWidth="7"/>
 ...
 <p className="text-text-muted text-sm max-w-md mx-auto leading-relaxed">{feedback.summary}</p>
 </div>

 {/* topic breakdown */}
 {feedback.topicBreakdown?.length > 0 && (
 <div className="bg-bg-surface/70 border border-border-default rounded-xl p-5">
 <h3 className="text-sm font-bold text-text-secondary mb-4 flex items-center gap-2"><Target className="w-4 h-4 text-indigo-400"/> Topic Breakdown</h3>
 <div className="space-y-3.5">
 {feedback.topicBreakdown.map((t, i) => (
 <div key={i}>
 <div className="flex justify-between mb-1.5">
 <span className="text-sm text-text-muted">{t.topic}</span>
 ...
 {t.comment && <p className="text-[11px] text-text-muted mt-1">{t.comment}</p>}
 </div>
 ))}
 </div>
 </div>
 ...
 <ul className="space-y-2.5">{feedback.strengths?.map((s,i) => <li key={i} className="flex items-start gap-2.5 text-sm text-text-secondary"><span className="text-emerald-500 mt-0.5 flex-shrink-0">✓</span>{s}</li>)}</ul>
 </div>
 ...
 <ul className="space-y-2.5">{feedback.areasToImprove?.map((s,i) => <li key={i} className="flex items-start gap-2.5 text-sm text-text-secondary"><span className="text-red-500 mt-0.5 flex-shrink-0">✗</span>{s}</li>)}</ul>
 </div>
 </div>
...
export default function AIInterviewPage() {
 ...
 const { run: tourRun, completeTour } = useOnboardingTour('interview')
 ...
 try {
 const { data } = await generateInterviewFeedback({ questions: qs, answers: ans, evaluations: [], interviewType: config.interviewType, company: config.company, resume: config.resume || '' })
 ...
 <button onClick={() => navigate('/dashboard')} className="flex items-center gap-1.5 text-text-muted hover:text-text-secondary text-sm mb-5 transition-colors">
 ...
 <div>
 <h1 className="text-2xl font-bold text-text-primary">AI Mock Interview</h1>
 <p className="text-sm text-text-muted mt-0.5">Voice-powered practice with an AI interviewer</p>
 </div>
 </div>
 </div>
 ...
 <p className="text-text-secondary font-semibold text-base mb-1">Analysing your interview…</p>
 <p className="text-text-muted text-sm">Evaluating {questions.length} answers with AI</p>
 </div>
 </div>
 )}
