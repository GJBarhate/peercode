...
const COMPANY_GRADIENTS = {
 ...
 Apple: 'from-bg-overlay to-bg-overlay border-border-strong/20 hover:border-border-strong/40',
 ...
 Uber: 'from-bg-overlay to-bg-overlay border-border-strong/20 hover:border-border-strong/40',
...
export default function TracksPage() {
 ...
 {/* Hero Header */}
 <div className="relative overflow-hidden rounded-2xl border border-border-default bg-gradient-to-br from-sky-50 via-white to-white dark:from-gray-900 dark:via-gray-900 dark:to-sky-950/30 p-6 sm:p-8 mb-8">
 <div className="absolute top-0 right-0 w-96 h-96 bg-sky-500/10 rounded-full blur-3xl" />
 <div className="absolute bottom-0 left-0 w-64 h-64 bg-sky-500/5 rounded-full blur-3xl" />
 <div className="relative">
 <div className="flex items-center gap-3 mb-3">
 <Sparkles className="w-6 h-6 text-sky-400" />
 <h1 className="text-3xl sm:text-4xl font-bold text-text-primary dark:text-white">Learning Tracks</h1>
 </div>
 <p className="text-text-muted text-sm sm:text-base max-w-2xl">Master algorithms & data structures through structured problem sets — practice solo or paired with a partner in real-time</p>
 ...
 <div className="mb-6 rounded-xl border border-border-default bg-bg-surface p-5 group hover:border-sky-500/30 transition-colors">
 ...
 <div>
 <p className="text-sm font-semibold text-text-primary dark:text-white">Your overall progress</p>
 <p className="text-xs text-text-muted">{overallStats.totalCompleted} of {overallStats.totalProblems} problems completed</p>
 </div>
 </div>
 ...
 <p className="text-base font-bold text-text-primary dark:text-white">{recommendedTrack.name}</p>
 {recommendedTrack.description && <p className="text-xs text-text-muted line-clamp-1 mt-0.5">{recommendedTrack.description}</p>}
 </div>
 ...
 className="w-full pl-10 pr-4 py-3 bg-bg-surface border border-border-default rounded-xl text-sm text-text-primary placeholder-gray-400 dark:placeholder-text-muted focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500/50 transition-all" />
 </div>
 ...
 <p className="text-text-muted mt-4">No tracks available yet.</p>
 </>
 )}
 </div>
 ) : (
 ...
 className={`group relative overflow-hidden rounded-2xl border border-border-default bg-bg-surface dark:bg-gradient-to-br dark:from-gray-900 dark:via-gray-900 dark:to-gray-950 p-5 flex flex-col gap-4 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-sky-500/10 ${gradient}`}> 
 ...
 <h3 className="font-bold text-text-primary dark:text-white group-hover:text-sky-600 dark:group-hover:text-sky-300 transition-colors leading-tight">{track.name}</h3>
 {track.company && (
 ...
 <ChevronRight className="w-5 h-5 text-text-muted group-hover:text-sky-500 dark:group-hover:text-sky-400 group-hover:translate-x-0.5 transition-all flex-shrink-0 mt-1" />
 </div>

 {track.description && (
 <p className="relative text-sm text-text-muted leading-relaxed line-clamp-2">{track.description}</p>
 )}

 <div className="relative flex items-center gap-3 text-xs text-text-muted">
 <span className="flex items-center gap-1.5"><BookOpen className="w-3.5 h-3.5 text-text-muted" />{total} problems</span>
 {track.estimatedHours && <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-text-muted" />{track.estimatedHours}h</span>}
 </div>

 {user && total > 0 && (
 <div className="relative">
 <div className="flex items-center justify-between text-xs mb-1.5">
 <span className="text-text-muted font-medium">{completed}/{total} <span className="text-text-muted">completed</span></span>
