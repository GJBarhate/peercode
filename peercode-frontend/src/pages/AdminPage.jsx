...
export default function AdminPage() {
 ...
 <h1 className="mt-2 text-3xl font-bold text-text-primary">Platform Operations</h1>
 <p className="mt-1 text-sm text-text-muted">Monitor health, moderate content, and keep practice sessions running cleanly.</p>
 </div>
 <button
 onClick={loadAdminData}
 disabled={loading}
 className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-bg-surface border border-border-strong text-sm font-semibold text-text-secondary hover:bg-bg-elevated disabled:opacity-50"
 >
 ...
 <div className="flex gap-1 p-1 bg-bg-surface border border-border-default rounded-lg overflow-x-auto">
 ...
 <section className="xl:col-span-2 bg-bg-surface border border-border-default rounded-lg p-5">
 <div className="flex items-center justify-between mb-4">
 <h2 className="text-sm font-semibold text-text-secondary">Sessions, Last 7 Days</h2>
 <span className="text-xs text-text-muted">UTC</span>
 </div>
 <div className="h-72">
 <ResponsiveContainer width="100%" height="100%">
 <AreaChart data={sessionChart}>
 <XAxis dataKey="_id" tick={{ fill: 'var(--color-chart-axis)', fontSize: 11 }} tickLine={false} axisLine={false} />
 <YAxis allowDecimals={false} tick={{ fill: 'var(--color-chart-axis)', fontSize: 11 }} tickLine={false} axisLine={false} />
 <Tooltip contentStyle={{ backgroundColor: 'var(--color-chart-tooltip-bg)', border: '1px solid var(--color-chart-tooltip-border)', borderRadius: 8 }} />
 <Area type="monotone" dataKey="count" stroke="var(--color-brand)" fill="var(--color-brand)" fillOpacity={0.25} />
 </AreaChart>
 </ResponsiveContainer>
 </div>
 </section>
 ...
 <h2 className="text-sm font-semibold text-text-secondary mb-4">Problem Mix</h2>
 <div className="h-72">
 <ResponsiveContainer width="100%" height="100%">
 <BarChart data={difficultyChart}>
 <XAxis dataKey="name" tick={{ fill: 'var(--color-chart-axis)', fontSize: 11 }} tickLine={false} axisLine={false} />
 <YAxis allowDecimals={false} tick={{ fill: 'var(--color-chart-axis)', fontSize: 11 }} tickLine={false} axisLine={false} />
 <Tooltip contentStyle={{ backgroundColor: 'var(--color-chart-tooltip-bg)', border: '1px solid var(--color-chart-tooltip-border)', borderRadius: 8 }} />
 ...
 return (
 <div key={plan.name} className="flex items-center justify-between p-3 bg-bg-elevated/50 rounded-lg">
 ...
 <p className="text-xs text-text-muted">₹{plan.price}/mo × {plan.count}</p>
 </div>
 </div>
 <div className="text-right">
 <p className="font-bold text-text-primary">₹{revenue.toLocaleString('en-IN')}</p>
 ...
 <p className="text-xs text-text-muted">Total Paid Subscribers</p>
 </div>
 <div className="bg-bg-elevated/50 rounded-lg p-4 text-center">
 ...
 <p className="text-xs text-text-muted">Annual Run Rate</p>
 </div>
 </div>
 </section>
 </div>
 ...
 <th key={header} className="px-4 py-3 text-left text-xs font-semibold text-text-muted uppercase tracking-wider">{header}</th>
 ))}
 </tr>
 </thead>
 ...
 <Crown className="w-8 h-8 mx-auto mb-2 text-text-muted" />
 ...
 const planColor = planColors[u.subscription?.plan] || 'text-text-muted bg-bg-overlay'
 return (
 ...
 <td className="px-4 py-3 text-sm text-text-muted">{u.sessionCount || 0}</td>
 ...
 <h3 className="text-sm font-semibold text-text-secondary">All Users ({users.length})</h3>
 <button onClick={exportUsers} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-text-secondary bg-bg-elevated hover:bg-bg-overlay transition-colors">
 ...
 <tbody className="divide-y divide-border-default">
 {users.map(user => (
 <tr key={user._id} className="hover:bg-bg-elevated/50/30">
 <td className="px-5 py-4">
 <p className="text-sm font-semibold text-text-primary">{user.username}</p>
 <p className="text-xs text-text-muted">{user.email}</p>
 </td>
 <td className="px-5 py-4 text-sm text-text-secondary uppercase">{user.role}</td>
 <td className="px-5 py-4 text-sm text-text-secondary">{user.elo ?? 1200}</td>
 <td className="px-5 py-4">
 ...
 <td className="px-5 py-4 text-sm text-text-muted">{new Date(user.createdAt).toLocaleDateString()}</td>
 <td className="px-5 py-4">
 <button
 onClick={() => handleToggleBan(user._id)}
 disabled={actionId === user._id || user.role === 'admin'}
 className="inline-flex items-center gap-2 px-3 py-1.5 rounded bg-bg-elevated hover:bg-bg-overlay disabled:opacity-50 text-xs font-semibold text-text-secondary"
 >
 ...
 <section className="bg-bg-surface border border-border-default rounded-lg overflow-hidden">
 <div className="flex items-center justify-between px-5 py-3 border-b border-border-default">
 <h3 className="text-sm font-semibold text-text-secondary">All Problems ({filteredProblems.length})</h3>
 <div className="flex items-center gap-2">
 <button onClick={exportProblems} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-text-secondary bg-bg-elevated hover:bg-bg-overlay transition-colors">
 <Download className="w-3.5 h-3.5" /> Export CSV
 </button>
 ...
 <tr key={problem._id} className="hover:bg-bg-elevated/50/30">
 <td className="px-5 py-4">
 <p className="text-sm font-semibold text-text-primary">{problem.title}</p>
 <p className="text-xs text-text-muted">{problem.slug}</p>
 </td>
 <td className="px-5 py-4">
 <span className="capitalize text-sm text-text-secondary">{problem.difficulty}</span>
 </td>
 <td className="px-5 py-4">
 <span className={`px-2 py-1 rounded text-xs font-semibold ${problem.isActive ? 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'bg-bg-overlay text-text-muted'}`}>
 ...
 className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs font-medium text-text-muted bg-bg-elevated hover:bg-bg-overlay transition-colors">
 ...
 <article key={report._id} className="bg-bg-surface border border-border-default rounded-lg p-5">
 <div className="flex items-start justify-between gap-4">
 <div>
 <p className="text-xs font-semibold uppercase tracking-wider text-red-700 dark:text-red-300">{report.type}</p>
 <h3 className="mt-1 text-lg font-semibold text-text-primary">{report.problem?.title || 'Unknown problem'}</h3>
 <p className="mt-1 text-xs text-text-muted">Reported by {report.reportedBy?.username || 'Unknown'}</p>
 </div>
 <button
 ...
 <p className="mt-4 text-sm leading-6 text-text-secondary">{report.description}</p>
 </article>
 ))}
 {filteredReports.length === 0 && (
 <div className="lg:col-span-2 bg-bg-surface border border-border-default rounded-lg">
 ...
 <div className="bg-bg-surface border border-border-default rounded-xl shadow-2xl w-full max-w-sm p-6">
 ...
 <div>
 <h3 className="text-base font-semibold text-text-primary">Archive problem?</h3>
 <p className="text-sm text-text-muted mt-0.5">This will hide the problem from users.</p>
 </div>
 </div>
 <p className="text-sm font-medium text-text-secondary mb-5 bg-bg-elevated rounded-lg px-3 py-2">
 "{confirmDelete.title}"
 </p>
 <div className="flex items-center justify-end gap-3">
 <button
 onClick={() => setConfirmDelete(null)}
 className="px-4 py-2 rounded-lg text-sm font-medium text-text-secondary bg-bg-elevated hover:bg-bg-overlay transition-colors"
 >
 Cancel
 </button>
 <button
